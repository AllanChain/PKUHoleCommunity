...
export class AppSwitcher extends Component {
  ...
  render() {
    ...
    this.state.apps.dropdown.forEach((app) => {
      if (app[0] === cur_id) dropdown_cur_app = app;
    });
...
class LoginPopupSelf extends Component {
  ...
  do_input_token(set_token) {
    ...
    const token = this.input_token_ref.current.value;
    this.setState(
      {
        loading_status: 'loading',
      },
        ...
        <div className="pkuhelper-login-popup">
          ...
          <p>
            ...
            {PERM_SCOPES.map(([scope, title]) => (
              <label key={scope} className="perm-item">
                <input
                  type="checkbox"
                  checked={this.state.excluded_scopes.indexOf(scope) === -1}
                  onChange={(e) => {
                    const chk = e.target.checked;
                    this.setState((prevState) => {
                      const exc = prevState.excluded_scopes.slice();
                      if (chk) {
                        // remove from exc
                        const pos = exc.indexOf(scope);
                        if (pos !== -1) exc.splice(pos, 1);
                      } else {
                        // add to exc
                        exc.push(scope);
                      }
          ...
          <p>
            ...
            <span className="pkuhelper-login-type">
              <a onClick={(e) => this.do_sendcode('sms')}>&nbsp;短信&nbsp;</a>/
              <a onClick={(e) => this.do_sendcode('mail')}>&nbsp;邮件&nbsp;</a>
            </span>
          </p>
          <p>
            <label>
